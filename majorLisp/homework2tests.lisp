...
(defun subtraction-p (F)
  (and (listp F) (equal (subtraction-operator F) subtraction-symbol) (>= (length F) 3)))

(defun product-p (F)
  (and (listp F) (equal (product-operator F) product-symbol) (>= (length F) 3)))

(defun division-p (F)
  (and (listp F) (equal (division-operator F) division-symbol) (>= (length F) 3)))

(defun power-p (F)
  (and (listp F) (equal (power-operator F) power-symbol) (>= (length F) 3)))
...
(defun make-power (F G)
  (cond ((eq F 0) 0)
	((eq G 0) 1)
	((and (numberp F) (numberp G)) (* F (make-power F (- G 1)))) ;;;subtract 1 from G for reccursion
