...
(defun differentiate (F V)
  ...
  (cond ((constant-p F) (make-constant 0)) ;;if F is a constant eval 0
	...
	((sum-p F) ;;if sum
	 (make-sum (differentiate (sum-operand-1 F) V) 
			   (differentiate (sum-operand-2 F) V))) 

	((negation-p F) ;;if negation	
	;;E(BNF) handled because negation is called before subtraction
	 (make-negation (differentiate (rest F) V)))

	((subtraction-p F) ;;if difference/subtraction
	 (make-subtraction
	  (differentiate (subtraction-operand-1 F ) V) 
	  (differentiate (sum-operand-2 F) V)))
	...
	((division-p F)  ;;if division
	 (make-division
	  (make-subtraction
	   (make-product (division-denominator F) 
					 (differentiate (division-numerator F) V))
	   (make-product (division-numerator F) (differentiate 
					 (division-denominator F) V)))
